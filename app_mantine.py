...
def create_analysis_stepper(variety_id):
    analysis_stepper = dmc.Stepper(
        ...
        children=[
            dmc.StepperStep(
                label=dmc.Anchor("基本面模型分析", href=f"/variety/basis?variety_id={variety_id}", id="basis-step"),
                description="基差-库存-利润模型分析",
                children=[
                    dmc.Divider(variant="solid", label="辅助功能", labelPosition="center"),
                    dmc.Space(h=10),
                    dmc.NavLink(label="图标设置", id="chart-config", icon=DashIconify(icon="tabler:settings")),
                    dmc.NavLink(label="主成分分析", variant="subtle", id="pca-analysis", icon=DashIconify(icon="ri:pie-chart-2-line")),
                    dmc.NavLink(label="强化学习建议", variant="subtle", id="drl-analysis", icon=DashIconify(icon="prime:microchip-ai")),
                ],
            ),
            dmc.StepperStep(
