...
dash.register_page(__name__, path='/market_old', title='Futures Nexus: 市场全景')

blank_layout = html.Div([
    dmc.Text(id="current-path"),
    dmc.NavLink(id="open-chain-page"),
    dmc.NavLink(id="open-variety-page"),
    dmc.NavLink(id="open-all-variety"),
    dmc.MultiSelect(id='heat-map-switch'),
    ...
    dmc.Stack(
        [
            dmc.Group([
                ...
                dcc.RadioItems(
                    id="heat-map-switch",
                    options=[x for x in ["持仓量", "成交量"]],
                    value="持仓量",
                ),
            ]),            
            dmc.LoadingOverlay(
                [
                    dcc.Graph(id='market-heatmap-placeholder', config={'displayModeBar': False}),
                ],
                visible=False,
